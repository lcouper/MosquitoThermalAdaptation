...
data.PDR.MAR1 <- subset(data.PDR, Population == "MAR1")
data.PDR.MAR2 <- subset(data.PDR, Population == "MAR2")
...
# not very pretty to plot points for each population separately since so many are overlapping
...
data.PDR.MAR1.prior <- subset(data.PDR, Population != "MAR35")
data.PDR.MAR2.prior <- subset(data.PDR, Population != "MAR29")
...
#### 5. Fit PDR thermal responses with data-informed priors ####
...
# Set data 
data <- data.PDR.HOP
hypers <- PDR.HOP.prior.gamma.fits * 0.1
...
#save(PDR.HOP.out.inf, file = "jagsout_PDR.HOPp_inf.Rdata")
...
# Set data 
data <- data.PDR.MAR1
hypers <- PDR.MAR1.prior.gamma.fits * 0.1
...
# Set data 
data <- data.PDR.MAR2
hypers <- PDR.MAR2.prior.gamma.fits * 0.1
...
# Set data 
data <- data.PDR.WAW
hypers <- PDR.WAW.prior.gamma.fits * 0.1
...
# Set data 
data <- data.PDR.EUG
hypers <- PDR.EUG.prior.gamma.fits * 0.1
...
# Set data 
data <- data.PDR.PLA
hypers <- PDR.PLA.prior.gamma.fits * 0.1
...
# Set data 
data <- data.PDR.SB
hypers <- PDR.SB.prior.gamma.fits * 0.1
...
# Set data 
data <- data.PDR.JRA
hypers <- PDR.JRA.prior.gamma.fits * 0.1
...
# Set data 
data <- data.PDR.PAR
hypers <- PDR.PAR.prior.gamma.fits * 0.1
...
# Set data 
data <- data.PDR.POW
hypers <- PDR.POW.prior.gamma.fits * 0.1
...
###### Topt of PDR for each population ######
ToptHOP =Temp.xs[which.max(as.vector(PDR.HOP.out.inf$BUGSoutput$summary[6:(6 + N.Temp.xs - 1), "50%"]))] # 34.5
ToptMAR1 =Temp.xs[which.max(as.vector(PDR.MAR1.out.inf$BUGSoutput$summary[6:(6 + N.Temp.xs - 1), "50%"]))] # 32
ToptMAR2 =Temp.xs[which.max(as.vector(PDR.MAR2.out.inf$BUGSoutput$summary[6:(6 + N.Temp.xs - 1), "50%"]))] # 32
ToptWAW =Temp.xs[which.max(as.vector(PDR.WAW.out.inf$BUGSoutput$summary[6:(6 + N.Temp.xs - 1), "50%"]))] # 33
ToptEUG =Temp.xs[which.max(as.vector(PDR.EUG.out.inf$BUGSoutput$summary[6:(6 + N.Temp.xs - 1), "50%"]))] # 29
ToptPLA =Temp.xs[which.max(as.vector(PDR.PLA.out.inf$BUGSoutput$summary[6:(6 + N.Temp.xs - 1), "50%"]))] # 32.5
ToptSB =Temp.xs[which.max(as.vector(PDR.SB.out.inf$BUGSoutput$summary[6:(6 + N.Temp.xs - 1), "50%"]))] # 33
ToptJRA =Temp.xs[which.max(as.vector(PDR.JRA.out.inf$BUGSoutput$summary[6:(6 + N.Temp.xs - 1), "50%"]))] # 30
ToptPAR =Temp.xs[which.max(as.vector(PDR.PAR.out.inf$BUGSoutput$summary[6:(6 + N.Temp.xs - 1), "50%"]))] # 32
ToptPOW =Temp.xs[which.max(as.vector(PDR.POW.out.inf$BUGSoutput$summary[6:(6 + N.Temp.xs - 1), "50%"]))] # 33
...
TbreadthHOP = Tbreadth(PDR.HOP.out.inf) # 22
TbreadthMAR1 = Tbreadth(PDR.MAR1.out.inf) # 21
TbreadthMAR2 = Tbreadth(PDR.MAR2.out.inf) # 21
TbreadthWAW = Tbreadth(PDR.WAW.out.inf) # 21.5
TbreadthEUG = Tbreadth(PDR.EUG.out.inf) # 19.5
TbreadthPLA = Tbreadth(PDR.PLA.out.inf) # 21
TbreadthSB = Tbreadth(PDR.SB.out.inf) # 21.5
TbreadthJRA = Tbreadth(PDR.JRA.out.inf) # 20
TbreadthPAR = Tbreadth(PDR.PAR.out.inf) # 21
TbreadthPOW = Tbreadth(PDR.POW.out.inf) # 22.5

#### Compile prediction data for all populations ###### 

PDRmeansd <- list(PDR.HOP.out.inf$BUGSoutput$summary[1:2,1:2], PDR.MAR1.out.inf$BUGSoutput$summary[1:2,1:2], 
                  PDR.MAR2.out.inf$BUGSoutput$summary[1:2,1:2], PDR.WAW.out.inf$BUGSoutput$summary[1:2,1:2], 
                  PDR.EUG.out.inf$BUGSoutput$summary[1:2,1:2], PDR.PLA.out.inf$BUGSoutput$summary[1:2,1:2],
                  PDR.SB.out.inf$BUGSoutput$summary[1:2,1:2], PDR.JRA.out.inf$BUGSoutput$summary[1:2,1:2],
                  PDR.POW.out.inf$BUGSoutput$summary[1:2,1:2], PDR.PAR.out.inf$BUGSoutput$summary[1:2,1:2])
PDRtops = as.data.frame(matrix(c(ToptHOP, ToptMAR1, ToptMAR2, ToptWAW, ToptEUG, ToptPLA, 
                                 ToptSB, ToptJRA, ToptPAR, ToptPOW, rep(NA, 10)), nrow = 10, ncol = 2))
PDRtbreadths = as.data.frame(matrix(c(TbreadthHOP, TbreadthMAR1,TbreadthMAR2, TbreadthWAW, TbreadthEUG, TbreadthPLA, 
                                     TbreadthSB, TbreadthJRA, TbreadthPAR, TbreadthPOW, rep(NA, 10)), nrow = 10, ncol = 2))
colnames(PDRtops) = c("mean", "sd")
colnames(PDRtbreadths) = c("mean", "sd")
PDRdf2 = do.call(rbind.data.frame, PDRmeansd)
PDRdf_1 = rbind(PDRdf2, PDRtops)
PDRdf = rbind(PDRdf_1, PDRtbreadths)
PDRdf$Population = c(rep(c("HOP", "MAR1", "MAR2", "WAW", "EUG", "PLA", "SB", "JRA", "PAR", "POW"), each = 2),
                     rep(c("HOP", "MAR1", "MAR2", "WAW", "EUG", "PLA", "SB", "JRA", "PAR", "POW"), 2))
PDRdf$Param = c(rep(c("Tmin", "Tmax"), 10), rep("Topt", 10), rep("Tbreadth", 10))
save(PDRdf, file = "PDR_meansd.Rsave")
...
PDRdf$Population = factor(PDRdf$Population, levels = PopOrder)
PDRdf = PDRdf[order(PDRdf$Population),]
PDRdf = PDRdf[PDRdf$Param != "Tbreadth",]


colors3 = rep(rev(c("#a50026","#d73027","#f46d43","#fdae61","#fee090","#e0f3f8","#abd9e9","#74add1", "#4575b4","#313695")), each = 3)

plotPDR = ggplot(PDRdf, aes(x=Population, y=mean, col = colors3)) + 
...
#### 6b. Plot pupal dev rates curve for all populations #####
...
plot(PupalDevRate ~ Temp.Treatment, 
     xlim = c(5, 45), ylim = c(0,0.4), data = data.LDR.HOP, type = "n", bty = "n",
